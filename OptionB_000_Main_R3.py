...
class VocabReaderApp(tk.Tk):
    def __init__(self):
        ...
        self.bubble_en_widgets: Dict[str, Dict] = {}
        self.bubble_vi_widgets: Dict[str, Dict] = {}
    ...
    def _apply_theme(self):
        ...
        self._refresh_bubble_appearance()
    ...
    def _refresh_bubble_appearance(self):
        for bubble in list(self.bubble_en_widgets.values()) + list(self.bubble_vi_widgets.values()):
            widget = bubble["widget"]
            frame = bubble["frame"]
            canvas = bubble["canvas"]
            number_item = bubble["number_item"]
            oval_item = bubble["oval_item"]
            font = tkfont.Font(font=widget["font"])
            line_height = font.metrics("linespace")
            descent = font.metrics("descent")
            diameter = max(18, line_height)
            frame_height = line_height + max(2, descent)
            frame.config(width=diameter + 6, height=frame_height, bg=widget.cget("bg"))
            frame.pack_propagate(False)
            canvas.config(width=diameter, height=diameter, bg=widget.cget("bg"))
            canvas.coords(oval_item, 0, 0, diameter, diameter)
            canvas.coords(number_item, diameter / 2, diameter / 2)
            number_size = max(8, int(self.font_size * 0.75))
            canvas.itemconfig(number_item, font=("Segoe UI", number_size, "bold"))
            canvas.itemconfig(oval_item, fill="#FFD000")
    ...
    def action_open_txt(self):
        ...
        self.bubble_en_widgets.clear()
        self.bubble_vi_widgets.clear()
    ...
    def delete_selected_word(self):
        ...
        self._remove_bubble_en(key)
        self._remove_bubble_vi(key)
        self._update_vietsub_highlights()
        self._update_bubble_numbers()
    ...
    def _apply_entry_highlight(self, key: str, entry: WordEntry):
        ...
        bubble_mark = f"bubble_en_{key}"
        self.text_en.mark_set(bubble_mark, start_index)
        self.text_en.mark_gravity(bubble_mark, tk.LEFT)
        bubble = self._create_bubble_widget(self.text_en)
        self.text_en.window_create(bubble_mark, window=bubble["frame"])
        word_start = start_index
        surface = entry.surface or self.text_en.get(word_start, end_index)
        word_end = self.text_en.index(f"{word_start}+{len(surface)}c")
        self.text_en.tag_add(self._status_to_tag(entry.status), word_start, word_end)
        start_mark = f"start_{key}"
        end_mark = f"end_{key}"
        self.text_en.mark_set(start_mark, word_start)
        ...
        self.bubble_en_widgets[key] = bubble
    ...
    def _remove_entry_highlight(self, key: str, entry: WordEntry):
        ...
        if marks:
            ...
            self.text_en.tag_remove("word_proficient", start_mark, end_mark)
            self.text_en.tag_remove("word_review", start_mark, end_mark)
            self.text_en.mark_unset(start_mark)
            self.text_en.mark_unset(end_mark)

    def _create_bubble_widget(self, widget: tk.Text) -> Dict[str, object]:
        font = tkfont.Font(font=widget["font"])
        line_height = font.metrics("linespace")
        diameter = max(18, line_height - 2)
        frame = tk.Frame(widget, width=diameter + 6, height=line_height, bg=widget.cget("bg"))
        frame.pack_propagate(False)
        canvas = tk.Canvas(frame, width=diameter, height=diameter, highlightthickness=0, bd=0, bg=widget.cget("bg"))
        canvas.pack(expand=True)
        oval_item = canvas.create_oval(0, 0, diameter, diameter, fill="#FFD000", outline="")
        ...
        canvas.bind("<Button-1>", lambda _: widget.focus_set())
        return {
            "widget": widget,
            "frame": frame,
            "canvas": canvas,
    ...
    def _remove_bubble_en(self, key: str):
        bubble = self.bubble_en_widgets.pop(key, None)
        if bubble:
            mark = f"bubble_en_{key}"
            self.text_en.delete(mark)
            bubble["frame"].destroy()
            self.text_en.mark_unset(mark)

    def _remove_bubble_vi(self, key: str):
        bubble = self.bubble_vi_widgets.pop(key, None)
        if bubble:
            mark = bubble["mark"]
    ...
    def _status_to_tag(self, status: str) -> str:
        mapping = {"new": "word_new", "proficient": "word_proficient", "review": "word_review"}
        return mapping.get(status, "word_new")
    ...
    def _reapply_highlights_en(self):
        ...
        self.text_en.tag_remove("word_proficient", "1.0", "end")
        self.text_en.tag_remove("word_review", "1.0", "end")
        for bubble in self.bubble_en_widgets.values():
            bubble["frame"].destroy()
        self.bubble_en_widgets.clear()
        self.entry_marks_en.clear()
    ...
    def _update_bubble_numbers(self):
        ...
        for key, bubble in self.bubble_en_widgets.items():
            number = order_map.get(key, "")
            bubble["canvas"].itemconfig(bubble["number_item"], text=str(number) if number else "")
        for key, bubble in self.bubble_vi_widgets.items():
            number = order_map.get(key, "")
            bubble["canvas"].itemconfig(bubble["number_item"], text=str(number) if number else "")
        self._refresh_bubble_appearance()
    ...
    def _clear_vietsub_state(self):
        ...
        self.text_vi.tag_remove("word_proficient", "1.0", "end")
        self.text_vi.tag_remove("word_review", "1.0", "end")
        for bubble in self.bubble_vi_widgets.values():
            bubble["frame"].destroy()
        self.bubble_vi_widgets.clear()
    ...
    def _update_vietsub_highlights(self):
        ...
        for entry in sorted_entries:
            ...
            self.text_vi.tag_add(self._status_to_tag(entry.status), start, end)
            mark = f"bubble_vi_{key}"
            self.text_vi.mark_set(mark, start)
            self.text_vi.mark_gravity(mark, tk.LEFT)
            bubble = self._create_bubble_widget(self.text_vi)
            self.text_vi.window_create(mark, window=bubble["frame"])
            bubble["mark"] = mark
            self.bubble_vi_widgets[key] = bubble
        self._update_bubble_numbers()
